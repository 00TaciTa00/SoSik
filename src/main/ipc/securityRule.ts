...
import { assertNonEmptyString, assertPositiveInt, validateSecurityPattern } from './validate'

export function registerSecurityRuleHandlers(): void {
  ipcMain.handle('security-rule:get-by-repo', (_event, repoId: unknown) => {
    assertNonEmptyString(repoId, 'repoId')
    logger.debug('IPC security-rule:get-by-repo', { repoId })
    return getRulesByRepo(repoId)
  })

  ipcMain.handle('security-rule:add', (_event, repoId: unknown, pattern: unknown) => {
    assertNonEmptyString(repoId, 'repoId')
    validateSecurityPattern(pattern)
    logger.debug('IPC security-rule:add', { repoId, pattern })
    return addRule(repoId, pattern)
  })

  ipcMain.handle('security-rule:remove', (_event, id: unknown) => {
